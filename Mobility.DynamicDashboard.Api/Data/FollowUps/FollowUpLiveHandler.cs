...
/// <summary>
/// Read-only opportunity follow-up handler shared by the current-follow-up and
/// all-follow-up dashboards. It owns the legacy parent/detail join and converts
/// unstable ASMX display lines into the normalized renderer fields.
...
/// </summary>
public sealed class FollowUpLiveHandler(FollowUpScopeResolver scopeResolver)
{
    public enum FollowUpDataSet
    {
        Current,
        All
    }
    ...
    public async Task<DashboardRowsQueryResult> QueryRowsAsync(
        ...
        FollowUpLiveOptions liveOptions,
        ILegacyFollowUpSource source,
        FollowUpDataSet dataSet,
        CancellationToken cancellationToken)
    {
        var scope = scopeResolver.Resolve(liveOptions, callerId, request.Context, false);
    ...
    public async Task<DashboardFilterOptionsResponse> GetFilterOptionsAsync(
        ...
        CancellationToken cancellationToken)
    {
        var scope = scopeResolver.Resolve(liveOptions, callerId, null, true);
        var snapshot = await LoadSnapshotAsync(
            scope,
            source,
            dataSet,
    ...
    public async Task<NormalizedDashboardRow?> FindRowAsync(
        ...
        CancellationToken cancellationToken)
    {
        var scope = scopeResolver.Resolve(liveOptions, callerId, context, false);
        var snapshot = await LoadSnapshotAsync(
            scope,
            source,
            dataSet,
    ...
    private async Task<CurrentOppSnapshot> LoadSnapshotAsync(
        ...
        CancellationToken cancellationToken)
    {
        // Match the two Flutter tiles exactly: Opportunity Follow-Up starts at
        // Opportunitie_List, while Current Opp All Followups starts at the
        // distinct Opportunitie_Mgt_List management parent/detail flow.
        var rootRows = await source.GetRootRowsAsync(scope, cancellationToken);
        var normalized = new List<NormalizedDashboardRow>();

        if (LooksLikeDetailRows(rootRows) || LooksLikeCurrentFollowUpRows(rootRows))
        {
            normalized.AddRange(rootRows.Select(row => NormalizeRow(row, null)));
        }
        else
        {
            var parentsByAgent = rootRows
                .Select(parent => new
                {
                    Parent = parent,
            ...
            // Flutter uses Future.wait for these approved detail calls. Keep
            // the API adapter behavior equivalent so the management list does
            // not serialize every parent round trip.
            var detailGroups = await Task.WhenAll(parentsByAgent.Select(async group =>
            {
                ...
                return detailRows.Select(detail =>
                {
            ...
            normalized.AddRange(detailGroups.SelectMany(rows => rows));
        }
    ...
    private static bool LooksLikeDetailRows(
        IReadOnlyList<LegacyFollowUpRow> rows)
    {
    ...
    private static bool LooksLikeCurrentFollowUpRows(
        IReadOnlyList<LegacyFollowUpRow> rows) =>
        rows.Count > 0 && rows[0].Values.Keys.Any(key =>
            key.Equals("Followup_Date", StringComparison.OrdinalIgnoreCase) ||
            key.Equals("FollowUpDate", StringComparison.OrdinalIgnoreCase) ||
            key.Equals("Customer_Name", StringComparison.OrdinalIgnoreCase) ||
            key.Equals("SalesPerson_Name", StringComparison.OrdinalIgnoreCase));

    private static string AgentIdFromParent(LegacyFollowUpRow parent) =>
        FirstNonEmpty(Read(parent, "Destination"), Read(parent, "ID"));

    private static string DestinationId(LegacyFollowUpRow parent)
    {
    ...
    private static bool IsManagementDetailParent(LegacyFollowUpRow parent) =>
        Read(parent, "NxtPg_Body").Contains(
            "opportunitie_mgt_detaillist",
            StringComparison.OrdinalIgnoreCase);

    private static bool IsStageGroupParent(LegacyFollowUpRow parent)
    {
    ...
    private static LegacyFollowUpRow? BestParentForDetail(
        IReadOnlyList<LegacyFollowUpRow> parents,
        LegacyFollowUpRow detail)
    {
        var bestScore = 0;
        LegacyFollowUpRow? best = null;
    ...
    private sealed record ParentContext(
        string AgentId,
        string AgentName,
        LegacyFollowUpRow? MatchedOrStageParent,
        LegacyFollowUpRow? MatchedParent);
